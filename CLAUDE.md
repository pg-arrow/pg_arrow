...
Use `just pg-setup <version>` from `pg_arrow/` (or `pgfusion/`). Set `PG_HARNESS_DIR` once; both crates share the same testdata + config in that directory.

```bash
export PG_HARNESS_DIR=/path/to/utils/pg-test-harness

# Full setup: build, init, load pgbench test data
just pg-setup pg18

# Simple schema instead of pgbench
...
Installs live under `$PG_HARNESS_DIR/testdata/postgres-<version>/`. No root/postgres user needed.

### `pg-test-config.toml` Format

Lives at `$PG_HARNESS_DIR/pg-test-config.toml`. Each version is a `[postgres.<name>]` section. Paths are stored **relative** to the harness directory; `pg_test_harness::read_pg_config()` resolves them against that root at load time.
