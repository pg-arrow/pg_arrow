...
Use `just pg-setup <version>` from `pg_arrow/` (or `pgfusion/`). This delegates to the canonical setup script in `utilities/pg-test-harness/`.
...
# Direct invocation (TARGET_DIR controls where pg-test-config.toml is written)
TARGET_DIR="$(pwd)" TESTDATA_DIR="$(pwd)/testdata" \
    bash ../utilities/pg-test-harness/scripts/setup-postgres.sh -b pg18 -B -i -t
```
...
Generated by the harness setup script. Each version is a `[postgres.<name>]` section:
